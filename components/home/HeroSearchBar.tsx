...
export default function HeroSearchBar() {
    ...
    return (
        <>
            {/* Mobile / tablet / iPad Pro: stacked card layout */}
            <div className="flex flex-col gap-2 bg-white rounded-2xl shadow-xl p-3 max-w-lg xl:hidden">
                {/* Where to */}
                <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-gray-50">
                    <HugeiconsIcon icon={Location01Icon} size={18} strokeWidth={1.8} className="text-gray-400 shrink-0" />
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Where to?</p>
                        <input
                            type="text"
                ...
                <div className="flex gap-2">
                    {/* When */}
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 flex-1">
                        <HugeiconsIcon icon={Calendar01Icon} size={16} strokeWidth={1.8} className="text-gray-400 shrink-0" />
                        <div>
                            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">When?</p>
                            <p className="text-xs text-gray-800">Anytime</p>
                        </div>
                    </div>

                    {/* Travelers */}
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 flex-1">
                        <HugeiconsIcon icon={User02Icon} size={16} strokeWidth={1.8} className="text-gray-400 shrink-0" />
                        <div>
                            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Travelers</p>
                            <p className="text-xs text-gray-800">2 Travelers</p>
                        </div>
                    </div>
                </div>

                <button className="w-full bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-xl px-6 py-3 transition-colors">
                    Explore Itineraries →
                </button>
            </div>

            {/* Desktop: single pill layout */}
            <div className="hidden xl:flex items-center bg-white rounded-full shadow-xl overflow-hidden pr-2 py-2 max-w-2xl">
